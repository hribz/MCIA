...
void DiffLineManager::Initialize(std::string &DiffPath,
  ...
  while (std::getline(file, line)) {
    ...
    if (std::sscanf(line.c_str(), "%d,%d %d,%d", &x1, &y1, &line_start,
                    &line_count) != 4) {
...
DiffLineManager::StartAndEndLineOfDecl(const Decl *D) {
...
DiffLineManager::OriginFilenameAndLineNumberOfDecl(const Decl *D) {
  SourceLocation Loc = D->getLocation();
  if (!(Loc.isValid() && Loc.isFileID())) {
    return std::nullopt;
  }
...
bool DiffLineManager::isChangedLine(unsigned int line, unsigned int end_line) {
  ...
  auto it = std::lower_bound(
      DiffLines->begin(), DiffLines->end(), std::make_pair(line + 1, 0),
      [](const std::pair<int, int> &a, const std::pair<int, int> &b) {
        return a.first < b.first;
      });
  auto it_begin_and_end =
      [](__gnu_cxx::__normal_iterator<const std::pair<int, int> *,
                                      std::vector<std::pair<int, int>>>
             it) {
        auto it_begin = it->first;
        auto it_end = it->first + it->second - 1;
        // it->second 是变化的行数，但是 it->second == 0
        // 并不意味着没有发生变化，而是 it->first 行之后发生了删除
