...
class ProjectInfo:
    def __init__(self, projects_root_dir, project):
        ...
        self.src_dir = os.path.join(projects_root_dir, self.repo_name) # The directory to store source code.
        ...
        self.options: List[Option] = parse_options(project['config_options'], self.switch_values, self.build_type)
        self.prerequisites = get_if_exists(project, 'prerequisites', []) # The commands need to be executed before building the project.
        self.dry_run = get_if_exists(project, 'dry_run', False)
        self.must_make = get_if_exists(project, 'make', False)  # This project must built before analysis.
        self.must_gcc = get_if_exists(project, 'gcc', False)    # This project must built through gcc.
        self.env = get_if_exists(project, 'env', {})
        self.ignore_make_error = get_if_exists(project, "ignore make error", False)

        self.build_dir = f"{self.src_dir}_build" if self.out_of_tree else self.src_dir # The directory to build project.